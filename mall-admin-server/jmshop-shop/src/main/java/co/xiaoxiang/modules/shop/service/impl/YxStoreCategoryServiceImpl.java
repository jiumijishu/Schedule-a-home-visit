...
import co.xiaoxiang.modules.shop.repository.YxStoreCategoryRepository;
import co.xiaoxiang.modules.shop.repository.YxStoreProductRepository;
import co.xiaoxiang.modules.shop.service.YxStoreCategoryService;
import co.xiaoxiang.modules.shop.service.dto.YxStoreCategoryDTO;
import co.xiaoxiang.modules.shop.service.dto.YxStoreCategoryQueryCriteria;
import co.xiaoxiang.modules.shop.service.mapper.YxStoreCategoryMapper;
...
//        }
        ...
        List<YxStoreCategory> categoryList = yxStoreCategoryRepository.findParentsByList(id);
        //删除子分类
        for (YxStoreCategory yxStoreCategory : categoryList) {
            YxStoreCategory category = new YxStoreCategory();
            category.setId(yxStoreCategory.getId());
            List<YxStoreProduct> storeProduct = yxStoreProductRepository.findByStoreCategoryAndIsDel(category, 0);
            if (!storeProduct.isEmpty()) {
                throw new BadRequestException(yxStoreCategory.getCateName() + " 此该分类子分类下有商品,不能删除");
            }
            yxStoreCategoryRepository.delCategory(yxStoreCategory.getId());
        }
        ...
        List<YxStoreProduct> storeProduct = yxStoreProductRepository.findByStoreCategoryAndIsDel(category, 0);
        if (!storeProduct.isEmpty()) {
            throw new BadRequestException("该分类下有商品,不能删除");
        }
