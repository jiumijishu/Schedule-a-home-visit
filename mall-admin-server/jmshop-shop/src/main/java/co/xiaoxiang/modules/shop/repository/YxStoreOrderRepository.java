...
import co.xiaoxiang.modules.shop.service.dto.ChartDataDTO;
...
public interface YxStoreOrderRepository extends JpaRepository<YxStoreOrder, Integer>, JpaSpecificationExecutor {
    ...
    /**
     * and refund_status=0 and is_del=0 and paid=1
     * @param idList
     * @return
     */
    @Query(value = "SELECT *  FROM yx_store_order where order_id in ?1",nativeQuery = true)
    List<YxStoreOrder> findByIds(List<String> idList);

    @Query(value = "SELECT *  FROM yx_store_order where paid=0 and `status`=0 and  (UNIX_TIMESTAMP(NOW())-add_time)>(60*60*2)",nativeQuery = true)
    List<YxStoreOrder> findUnPayOrders();
}
