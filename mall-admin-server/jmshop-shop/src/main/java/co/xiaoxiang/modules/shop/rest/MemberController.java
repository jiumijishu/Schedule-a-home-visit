...
import co.xiaoxiang.exception.BadRequestException;
...
import co.xiaoxiang.modules.activity.service.dto.YxStoreCouponIssueUserQueryCriteria;
import co.xiaoxiang.modules.shop.domain.YxUser;
import co.xiaoxiang.modules.shop.service.YxSystemConfigService;
import co.xiaoxiang.modules.shop.service.YxUserLevelService;
import co.xiaoxiang.modules.shop.service.YxUserService;
import co.xiaoxiang.modules.shop.service.dto.UserMoneyDTO;
import co.xiaoxiang.modules.shop.service.dto.YxUserDTO;
import co.xiaoxiang.modules.shop.service.dto.YxUserQueryCriteria;
...
public class MemberController {
    ...
    public ResponseEntity update(@Validated @RequestBody YxUser resources){
        ...
        if(resources.getYxSystemUserLevel()!=null && resources.getYxSystemUserLevel().getId()!=null) {
            ...
            yxUserService.removeUserLevel(resources.getUid());
            yxUserLevelService.removeUserLevel(resources.getUid());
        }
    ...
    @Log("设置用户标签")
    @ApiOperation(value = "修改用户")
    @PutMapping(value = "/yxUser/setUserLables")
    @PreAuthorize("@el.check('admin','YXUSER_ALL','YXUSER_EDIT')")
    public ResponseEntity setUserLables(@RequestBody YxUser resources){
        if(StringUtils.isEmpty(resources.getUids())){
            return new ResponseEntity(HttpStatus.NO_CONTENT);
        }
        String []userIds=resources.getUids().split(",");
        for(String uid:userIds){
            YxUserDTO userInfo= yxUserService.findById(Integer.parseInt(uid));
            YxUser editUser=new YxUser();
            if(StringUtils.isNotEmpty(userInfo.getUserLabel())){
                editUser.setUserLabel(userInfo.getUserLabel()+","+resources.getUserLabel());
            }else{
                editUser.setUserLabel(resources.getUserLabel());
            }
            editUser.setUid(Integer.parseInt(uid));
            yxUserService.update(editUser);
        }
        return new ResponseEntity(HttpStatus.NO_CONTENT);
    }
