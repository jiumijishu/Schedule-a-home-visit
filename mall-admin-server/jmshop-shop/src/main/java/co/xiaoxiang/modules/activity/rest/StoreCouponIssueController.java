package co.xiaoxiang.modules.activity.rest;
...
public class StoreCouponIssueController {
    ...
    @GetMapping(value = "/yxStoreCouponIssue/registerSend/{id}")
    public ResponseEntity registerSend(@PathVariable Integer id) {
        YxStoreCouponIssueDTO coupon= yxStoreCouponIssueService.findById(id);
        if(coupon==null){
            return new ResponseEntity("优惠券不存在",HttpStatus.NO_CONTENT);
        }
        YxStoreCouponIssue resource=new YxStoreCouponIssue();
        resource.setRegisterFlag("Y");
        resource.setId(id);
        yxStoreCouponIssueService.update(resource);
        return new ResponseEntity(HttpStatus.OK);
    }
}
