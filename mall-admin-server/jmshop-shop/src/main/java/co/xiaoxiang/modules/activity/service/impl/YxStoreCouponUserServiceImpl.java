...
import co.xiaoxiang.modules.activity.repository.YxStoreCouponUserRepository;
...
import co.xiaoxiang.modules.activity.service.dto.YxStoreCouponUserQueryCriteria;
import co.xiaoxiang.modules.activity.service.mapper.YxStoreCouponUserMapper;
...
public class YxStoreCouponUserServiceImpl implements YxStoreCouponUserService {
    ...
    public void autoNoticeExpireUser() {
        ...
        couponList.stream().forEach(userCoupon->{
            ...
            if(userCoupon.getEndTime()> currentTIme){
               int hour= (userCoupon.getEndTime()-currentTIme);
               if(hour>0 && hour<=86400){
                   ...
                   //修改提醒状态
                   Optional<YxStoreCouponUser> yxStoreCouponUser = yxStoreCouponUserRepository.findById(userCoupon.getId());
                   ValidationUtil.isNull(yxStoreCouponUser,"YxStoreCouponUser","id",userCoupon.getId());
                   YxStoreCouponUser couponUser= yxStoreCouponUser.get();
                   couponUser.setRemindFlag("Y");
    ...
    public static void main(String[] args) {
        long endtime=OrderUtil.getSecondTimestampTwo()+86400;
